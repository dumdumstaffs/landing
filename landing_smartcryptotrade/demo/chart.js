...
Chart.prototype.init = function() {
  ...
  this.candlesPerPage = Math.ceil(
    this.mainCanvas.clientWidth / (this.candlesFreeSpace * 2 + this.candleWidth)
  );
  ...
  this.imageEnd = new Image();
  {
    ...
    buf.textAlign = "right";
    buf.fillText(
      this.config.imageDed.text,
      this.config.imageDed.textPosX,
      this.config.imageDed.textPosY
    );
    ...
    buf.textAlign = "right";
    buf.fillText(
      this.config.imageEnd.text,
      this.config.imageEnd.textPosX,
      this.config.imageEnd.textPosY
    );
  ...
  this.mainCanvas.onmouseup = function() {
    if ($this.dragging) {
      $this.speed = Math.round(
        (1000 * $this.translateTo) /
          Math.pow($this.now - $this.translateTime, 2)
      );
    }
    $this.dragging = false;
  };
...
Chart.prototype.scrollX = function(delta) {
  ...
  if (
    ...
    $this.candleWidth + $this.candlesFreeSpace * 2
  ) {
    var movCandle = Math.floor(
      $this.offset / ($this.candleWidth + $this.candlesFreeSpace * 2)
    );
    ...
    $this.offset -=
      movCandle * ($this.candleWidth + $this.candlesFreeSpace * 2);
  }
};
Chart.prototype.getX = function(time) {
  ...
  if (lc.timeId < time) {
    return (
      this.lastPosX +
      ((this.candleWidth + this.candlesFreeSpace * 2) / 1000) *
        (time - this.now)
    );
  }
  return 0;
...
Chart.prototype.getTime = function(x) {
  ...
  if (lc.posX < x) {
    ...
    return (
      this.now + (1000 * dd) / (this.candleWidth + this.candlesFreeSpace * 2)
    );
  }
};
...
Chart.prototype.animate = function() {
  ...
  this.candlesFreeSpace +=
    (this.candlesFreeSpaceTo - this.candlesFreeSpace) / 5;
  this.candleWidth += (this.candleWidthTo - this.candleWidth) / 5;
  this.candlesPerPage = Math.ceil(
    this.clientWidth / (this.candlesFreeSpace * 2 + this.candleWidth)
  );
  ...
  if (
    this.preparedOrder &&
    this.preparedOrder.timeFrame &&
    (!this.preparedOrder.optionEnd || this.now >= this.preparedOrder.optionEnd)
  ) {
    //if(!this.optionEnd) {
    this.preparedOrder.optionOff = this.now + this.preparedOrder.timeFrame;
    this.preparedOrder.optionOff =
      Math.round(this.preparedOrder.optionOff / 1000) * 1000;
    this.preparedOrder.optionEnd = Math.round(
      this.now + this.preparedOrder.timeFrame - this.preparedOrder.timeOff
    );
    this.preparedOrder.optionEnd =
      Math.round(this.preparedOrder.optionEnd / 1000) * 1000;
  }

  this.endLine.draw(this.preparedOrder.optionEnd);

  var text = moment(this.preparedOrder.optionEnd - Date.now())
    .utc()
  ...
  this.mousePosPrice.draw(
    candle.close,
    this.getY(candle.close),
    this.config.currentPrice.bg
  );
  ...
  if (
    ...
    this.mouseX > 10 &&
    this.mouseX < this.width - 10
  ) {
    this.mousePosPrice.draw(
      this.getPrice(this.mouseY),
      this.mouseY,
      this.config.mousePos.bg
    );
...
Chart.prototype.drawTimes = function() {
  ...
  for (var px = step; px < this.width; px += step) {
    ...
    ctx.fillText(
      moment(this.getTime(px))
        .utc()
        .format(this.config.timeLine.text.format),
      px,
      this.height - 5
    );
  }
...
Chart.prototype.drawCandles = function() {
  ...
  this.candlesShow = [];
  for (
    var ci = this.candlesOffset;
    ci >= this.candlesOffset - this.candlesPerPage && ci >= 0;
    ci--
  ) {
  ...
  if (this.lastDrag + 2000 < this.now && lCandle.posX > this.width * 0.75) {
    if (
      this.candlesOffset <
      this.width / (this.candlesFreeSpace * 2 + this.candleWidth)
    ) {
      this.lastDrag = this.now;
      this.speed -=
        Math.abs(lCandle.posX - this.width * 0.5) /
        (this.candleWidth + this.candlesFreeSpace * 2);
...
TimeLine.prototype.draw = function(time) {
  ...
  var text = moment(time)
    .utc()
    .format(this.chart.config.currentTime.text.format);
  var w1 = this.ctx.measureText(text).width;
  this.ctx.beginPath();
  this.ctx.fillStyle = this.chart.config.currentTime.text.bg;
  this.ctx.fillRect(
    this.chart.mouseX - w1 / 2 - pagging,
...
EndLine.prototype.draw = function(
  ...
  if (price) {
    ...
    var closeText = moment(time)
      .utc()
      .format(this.chart.config.order.time.format);
...
Candle.prototype.draw = function(ctx, chart, nr, total) {
  var offset =
    nr * (chart.candleWidth + 2 * chart.candlesFreeSpace) + chart.offset;
  if (this.animation == 0) {
  ...
  if (nr == total - 1) {
    var wTotal =
      (chart.candleWidth + chart.candlesFreeSpace * 2) * 0.001 * mili;
    chart.lastPosX =
      offset + wTotal - chart.candleWidth - chart.candlesFreeSpace * 2;
  }
...
ch = {
  link: function(element) {
    ...
    setInterval(function(stomp) {
      ...
      var pp =
        cp.close +
        Math.round((Math.random() - 0.5 + markaPizdec * 0.1) * 4) / 1000;
      chart.onTick({
        timeId: Date.now(),
        close: pp,
        open: pp,
