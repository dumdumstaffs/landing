...
(function(){
    ...
    function listAllTags(order){
        ...
        $.ajax({
            url: settings.endpoint+'/course/tag/list?size=1000&page=1&with-categories=3&with-details=false&order='+order+'&locale='+locale,
            type: 'GET',
            headers: {
                'X-HEADER-HOST':'rTQkuyN2GnNgB//Tu3yHRYBWfkY=',
...
;            }
    ...
    function applyFilter(){
        // Declare variables
        var input, filter, tagName, count = 0;

        // Assign variables
        input = $('#tag-search-form input[type="text"]');
        ...
        if(input.val().length > 0){
            ...
            // Filter Tags - display those that match...
            ...
            tagName.each(function(){
                ...
                // Display all tags, increment counter, add counter to the view
                count++;
                $(this).closest('li').css('display', 'inline-block');
     ...
     */
    $('body').on('click', '#search-tags', applyFilter);
